...
})(({ record, dispatch, languages, ...props }) => {
  if (!record || !record.fk_languageId || !languages[record.fk_languageId]) {
    return null;
  }

  return <TextField {...props} source="Language.name" record={{ Language: languages[record.fk_languageId] }} />;
});
...
})(({ record, dispatch, topics, ...props }) => {
  if (!record || !record.fk_topicId || !topics[record.fk_topicId]) {
    return null;
  }

  return <TextField {...props} source="Topic.name" record={{ Topic: topics[record.fk_topicId] }} />;
});
