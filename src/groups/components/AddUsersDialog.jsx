...
import { useTranslate, TextInput, useNotify, useDataProvider, useRefresh, TextField } from 'react-admin';
import { Box, Button, Dialog, Typography, IconButton, Table, TableCell, TableBody, TableRow, TableHead, Checkbox } from '@material-ui/core';
import Alert from '@material-ui/lab/Alert';
import CloseIcon from '@material-ui/icons/Close';
import { useIsAdmin } from '../../hooks';

const Filters = ({ onSubmit, selected, onSelectedSubmit }) => {
      ...
      render={({ handleSubmit, form }) => {
        return (
          <form onSubmit={handleSubmit} autoComplete="off">
            <Box display="flex">
              <Box flex={3}>
                <Typography variant="body2">{translate('misc.search_users')}</Typography>
...
const ResultsList = ({ users, selected, toggleSelect }) => {
  const translate = useTranslate();
  ...
  if (!users.length) {
    return <Alert severity="info">{translate('resources.users.no_results')}</Alert>;
  }
      ...
      <Table>
        ...
        <TableBody>
          {users.map((user, i) => (
            <TableRow key={i}>
              <TableCell>
                <Checkbox checked={isSelected(user)} value={isSelected(user)} onClick={() => toggleSelect(user)} />
              </TableCell>
              <TableCell>
    ...
    <>
      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="lg">
        <Box display="flex" p={2}>
          <Box flex={5}>
            <Typography>{translate('resources.groups.users')}</Typography>
          </Box>
        ...
        <Box p={2}>
          <Filters onSubmit={onFiltersSubmit} selected={selected} onSelectedSubmit={onSelectedSubmit} />
          <hr />
          <ResultsList
            {...{
              users,
