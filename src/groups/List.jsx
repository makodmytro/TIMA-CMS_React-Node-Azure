...
function LinkTab(props) {
  return <Tab component={Link} style={{ textDecoration: 'underline', border: '2px solid white' }} {...props} />;
}
...
const WorkflowRole = ({ record }) => {
  ...
  return <TextField source="role" record={{ role: translate(`resources.users.workflow.roles.${match.name}`) }} />;
};
  ...
  return (
    <>
      <AppBar position="static" color="secondary">
        <Tabs variant="fullWidth" value={1} aria-label="nav tabs example" TabIndicatorProps={{ style: { background: 'white' } }}>
      ...
      <Box p={2} boxShadow={3}>
        <List
          ...
          actions={
            <ListActions visibleColumns={visibleColumns} onColumnsChange={handleColumnsChange('groups', setVisibleColumns)} columns={columns} />
          }
